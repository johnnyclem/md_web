...
(function($){
	...
	$.supersized = function( options ) {
		...
		var settings = {
			...
			set						: '72157632465720340', 	//Flickr set ID (found in URL)
	...
	switch(options.sort_direction){
		...
		function nextslide() {
			...
			// $('#prev_image').append($('<a href="'+options.slides[currentSlide].image+'"> <img src="'+options.slides[currentSlide].thumb+'" /> </a>'));
	    	...
	    	switch(options.transition){
	    		...
	    		case 0:    //No transition
	    		    ...
	    		    break;
	    		case 1:    //Fade'background-color', 'rgba(255,0,0,0.5)'
	    				// bgArray.push(options.slides[currentSlide].thumb);
	    				// var bgURL = 'url('+bgArray[0]+')';
	    				// for (var i = bgArray.length-1; i >= 0; i--) {
	    				// 	bgURL = bgURL+',url('+bgArray[i]+')';
	    				// };
							$('a.activeslide').css('background-color', '#fff');
							// $('a.activeslide').css('background-image', 'url('+options.slides[currentSlide].thumb+')');
							// console.log('new bgURL: '+$('a.activeslide').css('background-image'));
